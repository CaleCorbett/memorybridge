...
# MCP PROMPTS
# =============================================================================
# The MCP `prompts` endpoint lets clients (Claude Desktop, Continue, etc.)
# fetch a pre-built message block that is injected into the conversation
# *before* any tool call, with no model decision required.
#
# This directly addresses the discretionary-call gap: when a client is
# configured to include the `memory_context` prompt in its system prompt,
# memories are always present — the model cannot skip them.
#
# Claude Desktop usage (claude_desktop_config.json):
#   No extra configuration needed beyond enabling the MemoryBridge server.
#   Open a new conversation, click the "+" icon in the prompt bar, and select
#   "memory_context" from the MemoryBridge prompts list. For always-on
#   injection, paste the prompt into your Claude Desktop custom system prompt.
# =============================================================================

@mcp.prompt(
    name="memory_context",
    description=(
        "Inject the full MemoryBridge memory corpus as a system-turn context block. "
        "Include this prompt at the start of every conversation to make memories "
        "unconditionally available — no tool call required. "
        "Accepts an optional profile name (defaults to the active profile) and "
        "an optional max_tokens budget (default 4000)."
    ),
)
def memory_context(
    profile: str = None,
    ...
    """Return a formatted memory-context block for system-prompt injection.

    The returned string is a plain-text block — clients inject it directly into
    the system turn. It is intentionally NOT JSON so Claude can read it without
    a tool call and without parsing overhead.

    Format::

        --- MemoryBridge context (profile: default) ---
        [identity block if set]

        MEMORIES (N total, X tokens):
        • [category] content  [importance: high]
        • [category] content
        ...
        --- end memory context ---

    High-importance memories are flagged inline.  The block is token-budgeted
    and applies the same recency-decay ranking used by get_memory().
    """
    resolved_profile = profile or _active_profile()
    _store.ensure_profile(resolved_profile)
    profile_data = _store.get_profile(resolved_profile)

    memories = _store.get_memories(resolved_profile)
    ...
    # Build the block greedily within the token budget.
    # Reserve ~120 tokens for the header/footer/identity block.
    HEADER_RESERVE = 120
    budget = max(max_tokens - HEADER_RESERVE, 0)
    ...
    for mem in memories:
        ...
        flag   = "  ⚑ high-importance" if imp == "high" else ""
        line   = f"• [{cat}] {content}{flag}"
    ...
    parts: list[str] = []
    parts.append(
        f"--- MemoryBridge context (profile: {resolved_profile}) ---"
    )

    # Identity block — name, role, or any top-level description
    if profile_data:
        identity = profile_data.get("identity") or {}
        identity_parts = []
        if identity.get("name"):
            identity_parts.append(f"User: {identity['name']}")
        if identity.get("role"):
            identity_parts.append(f"Role: {identity['role']}")
        if identity.get("description"):
            identity_parts.append(identity["description"])
        if identity_parts:
            parts.append("\n".join(identity_parts))

    parts.append(
        f"\nMEMORIES ({included} of {total}, ~{tokens_used} tokens):"
    )
    if lines:
        parts.extend(lines)
    else:
    ...
    _store.log_access(
        "memory_context_prompt", resolved_profile,
        f"budget={max_tokens}, included={included}/{total}",
        tokens_served=count_tokens(block),
    )
    return block
